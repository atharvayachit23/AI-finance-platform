...
const HeroSection = () => {
  ...
  return (
    <section className="relative pt-48 pb-32 px-4 overflow-hidden">
      ...
      <div className="absolute top-20 left-1/2 -translate-x-1/2 w-[600px] h-[300px] bg-primary/10 blur-[120px] rounded-full -z-10" />
      ...
      <div className="container mx-auto text-center">
        ...
        {/* Hero Title - Aggressive & Custom */}
        <h1 className="text-6xl md:text-8xl lg:text-[115px] font-black tracking-tighter text-white mb-8 uppercase italic leading-[0.85]">
          Manage <br />
        ...
        <div className="flex flex-col sm:flex-row justify-center items-center space-y-4 sm:space-y-0 sm:space-x-4 mb-24">
          <Link href="/dashboard">
            <Button size="lg" className="h-16 px-10 rounded-full bg-primary text-black font-black hover:bg-emerald-400 shadow-glow hover:scale-105 transition-all border-none group">
              ACCESS_TERMINAL <ChevronRight className="ml-2 h-5 w-5 group-hover:translate-x-1 transition-transform" />
            </Button>
          </Link>
          <Link href="#features">
            <Button variant="outline" size="lg" className="h-16 px-10 rounded-full border-white/10 bg-white/5 text-white hover:bg-white/10 transition-all font-bold">
              SYSTEM_PROTOCOL
            </Button>
          </Link>
        </div>
        ...
        <div className="hero-image-wrapper mt-10 md:mt-0">
          ...
          >
            {/* Overlay indicators to make the image look like a real "System" */}
            ...
            <div className="relative rounded-[2rem] overflow-hidden bg-black border border-white/5 aspect-video">
              <Image
                ...
                priority
              />
              {/* Dark Gradient Overlay to hide the "stock image" feel */}
